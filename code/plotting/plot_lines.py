...
def plot_map(stations, connections, lines, area, output_path='./output/'):
    ...
    for connection in connections:
        connect_long = [connection._start._longitude, connection._end._longitude]
        connect_lat = [connection._start._latitude, connection._end._latitude]
        ax.plot(connect_lat,connect_long, linewidth=line_size, color='grey', zorder=1)

    # Plot all stations
    station_long = [station._longitude for station in stations.values()]
    station_lat = [station._latitude for station in stations.values()]
    ...
    # Plot all lines
    for station_line in [line._stations for line in lines]:
        line_long = [station._longitude for station in station_line]
        line_lat = [station._latitude for station in station_line]
    ...
    if isinstance(area,str):
        ...
        if area == 'Holland':
            ...
            if world.name.str.contains(area).sum() > 0:
                ...
                sys.exit('No match to that area found. Try other name.')
                
    else:
        sys.exit('Please insert String.')
